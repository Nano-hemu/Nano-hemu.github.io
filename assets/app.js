(()=>{
  ...
  if(!matchMedia('(prefers-reduced-motion: reduce)').matches){
    ...
    function reset(){
      crystals=Array.from({length:24},()=>({
        x:rnd(.035*w,.965*w),
        y:rnd(.05*h,.95*h),
        r:rnd(2.2,4.5)*dpr,
        max:rnd(26,62)*dpr,
        a:rnd(0,tau),
        arms:Array.from({length:6},()=>rnd(.58,1.2))
      }));
    }
    function resize(){
    ...
    function dendrite(c,alpha){
      ctx.strokeStyle=isDark()?'rgba(158,132,255,'+alpha+')':'rgba(83,79,205,'+alpha+')';
      ctx.lineWidth=.72*dpr;
      for(let k=0;k<6;k++){
        ...
        const bn=Math.max(1,Math.floor(L/(15*dpr)));
        for(let j=1;j<=bn;j++){
          const t=j/(bn+1),bx=c.x+(x2-c.x)*t,by=c.y+(y2-c.y)*t,bl=Math.min(11*dpr,L*.23);
          for(const s of[-1,1]){
    ...
    function frame(now){
      ...
      for(const p of solutes){
        ...
        ctx.fillStyle=isDark()?'rgba(110,185,255,.12)':'rgba(40,91,190,.11)';
        ctx.beginPath();ctx.arc(p.x,p.y,1.0*dpr,0,tau);ctx.fill();
      }
      for(const c of crystals){
        c.r+=.0105*dt;
        if(c.r>c.max){
          c.r=rnd(2.2,4.5)*dpr;c.x=rnd(.035*w,.965*w);c.y=rnd(.05*h,.95*h);c.a=rnd(0,tau);
        }
        dendrite(c,.12+Math.min(.12,c.r/(210*dpr)));
      }
